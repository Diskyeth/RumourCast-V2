        ...
        <div className="flex flex-row gap-4 justify-between">
              ...
              <div className="text-sm font-medium text-zinc-400">
                ...
                {cast.credentials.map((c) => {
                  ...
                  return (
                    ...
                    >
                      <Coins size={12} />
                      {`${formatNumber(Number.parseFloat(formatEther(BigInt(c.metadata.balance))))} ${c.metadata.tokenAddress?.toLowerCase() === TOKEN_ADDRESS.toLowerCase() ? 'RUMOUR' : c.metadata.tokenAddress}`}
                    </div>
                  )
                })}
                {cast.parent_hash && (
          ...
          {cast.embeds.map((embed) => {
            if (embed.metadata?.image) {
              return (
                <img key={embed.url} src={embed.url} alt="embed" className="rounded-md" />
              )
            }
            if (embed.metadata?.html) {
              return (
                <div key={embed.url} className="w-full border rounded-md overflow-hidden">
